...
**What the search does not reach, and why.** It builds a unit as the orbit of
**one** cell under **one** cyclic group of turns. That reaches exactly the
tilings that are *isohedral* -- one orbit of tiles -- and whose symmetry is
generated by rotations. The article gives every type's wallpaper group and
primitive unit size, and the orbit count follows from dividing:

| type | group | unit | orbits | |
|---|---|---|---|---|
| 1 | p2 | 2 | 1 | found |
| 2 | pgg | 4 | 1 | needs a flip |
| 3 | p3 | 3 | 1 | found |
| 4 | p4 | 4 | 1 | found |
| 5 | p6 | 6 | 1 | found |
| 6 | p2 | 4 | 2 | needs two seeds |
| 7 | pgg | 8 | 2 | both |
| 8 | pgg | 8 | 2 | both |
| 9 | pgg | 8 | 2 | both |
| 10 | p2 | 6 | 3 | needs three seeds |
| 11 | pgg | 8 | 2 | both |
| 12 | pgg | 8 | 2 | both |
| 13 | pgg | 8 | 2 | both |
| 14 | p2 | 6 | 3 | needs three seeds |
| 15 | pgg | 12 | 3 | both |

Orbits are the unit size over the order of the point group (2 for p2, 3 for
p3, 4 for p4 and for pgg, 6 for p6), and that agrees with the isohedrality
the article states in prose for types 2, 6-10 and the Rice types -- two
descriptions of the same thing, derived separately.

So the four the search finds are **exactly** the four that are isohedral and
rotation-generated. Nothing about it is approximate, and the size of the unit
is not the obstacle: type 6's unit is four cells, the same size the two-half-
turns family builds, and it is still out of reach because those four cells are
two orbits rather than one.

Two capabilities are missing, and they are separable. A **reflected copy**, for
the seven pgg types, whose single orbit contains mirror images that no turn
produces -- type 2 needs only this. And a **second and third seed cell**, for
the eight types that are 2- or 3-isohedral, since their unit cannot be any one
cell's orbit. Glide reflections were tried on their own and found nothing at
eight times the running time, which now makes sense: of the seven pgg types,
only type 2 is within reach of a flip alone.

Separately, types 7, 11 and 12 fail earlier than any of this -- the solver
does not close a pentagon for them at all. That is a matter of the free
parameters chosen and where Newton is started, not of arrangements.
